 ...
 *
 * **Unconfined**, so a `collectSafely` started inside `init` — `SimilarPhotosViewModel` has one —
 * has already run by the time the constructor returns, which is what the screen sees too.
 */
