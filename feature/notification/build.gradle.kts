// :feature:notification — android-library + Compose. LLM.md §3.7.
...
android {
    namespace = "com.pion.phonecleaner.feature.notification"
...
dependencies {
    ...
    // App-icon rows on the hiding-settings list and both permission tabs. `AppIconLoader`
    // (`coreUiModule`) owns the ImageLoader and the fetcher; this module needs the `AsyncImage`
    // composable to hand it a request (`:core:ui` keeps Coil `implementation`, so it is not on this
    // module's compile classpath through the project edge).
