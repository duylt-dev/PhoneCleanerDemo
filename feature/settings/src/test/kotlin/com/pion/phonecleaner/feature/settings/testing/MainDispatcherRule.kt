 ...
 * runtime (`LLM.md` §9). Copied in shape from `feature/files/.../testing/MainDispatcherRule.kt`; a
 * fake needed by two modules is `LLM.md` §9's one unresolved testing question, so it is duplicated
 * rather than promoted on this change's authority.
 *
 * **Unconfined**, so a `launch` raised inside `init` has already run by the time the constructor
 * returns — which is what the screen sees too.
 */
...
/** A **bounded** advance: `advanceUntilIdle` on a ViewModel holding a ticker never returns. */
@OptIn(ExperimentalCoroutinesApi::class)
