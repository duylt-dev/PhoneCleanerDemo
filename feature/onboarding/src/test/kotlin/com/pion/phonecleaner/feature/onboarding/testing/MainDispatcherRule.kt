...
/**
 * A **bounded** advance. `advanceUntilIdle` on a ViewModel holding a ticker never returns — the
 * scheduler is never idle, so the call hangs the suite instead of failing it (`LLM.md` §9).
 */
@OptIn(ExperimentalCoroutinesApi::class)
