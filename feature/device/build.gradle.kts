// :feature:device — android-library + Compose. LLM.md §3.7.
...
android {
    namespace = "com.pion.phonecleaner.feature.device"
...
dependencies {
    ...
    // AppIconLoader (:core:ui) hands the row a Coil `ImageLoader` and a request key, and the row
    // draws it with `AsyncImage`. :core:ui keeps coil as `implementation`, so the composable that
    // calls AsyncImage needs its own dependency — the same line :feature:files carries.
